...
function PANEL:Init()
    ...
    for i = 0, 0 do
        ...
        for k, v in ipairs(CustomShipments) do
            ...
            ListItem.Paint = function(self, w, h)
                ...
                if Arivia.TabWeapons.xpsystemEnabled and LevelSystemConfiguration then
                ...
                if LevelSystemPrestigeConfiguration and Arivia.TabWeapons.prestigeEnabled then
                ...
                if (Arivia.TabWeapons.xpsystemEnabled and ( LevelSystemConfiguration ) ) or (Arivia.TabWeapons.prestigeEnabled and ( LevelSystemPrestigeConfiguration or v.prestige ) )  then
                    originNameX = 11
                end
                ...
                draw.DrawText( objectName, "AriviaFontObjectListName", 65, originNameX, Arivia.TabWeapons.nameColor or Color( 255, 240, 244 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )

                if Arivia.TabWeapons.xpsystemEnabled then
                    if LevelSystemConfiguration then
                        ...
                        if Arivia.TabWeapons.prestigeEnabled and LevelSystemPrestigeConfiguration then
                            draw.DrawText( prestigeText, "AriviaFontObjectLevel", 140, 35, Color( 255, 240, 244, 100 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
                        end
                    end
                elseif Arivia.TabWeapons.prestigeEnabled then
                ...
                draw.DrawText( GAMEMODE.Config.currency .. ( v.pricesep or v.price ), "AriviaFontObjectPrice", w - 31, Arivia.Settings.ItemAmountH or 20, Color( 255, 255, 255, 120 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

                -----------------------------------------------------------------
                -- itemUnavailable - CHECK PLAYER LEVEL
                -----------------------------------------------------------------
                if LevelSystemConfiguration and Arivia.TabWeapons.xpsystemEnabled then
                    local itemLevel = v.level or v.lvl
    ...
    if Arivia.CustomEntityDescriptionsEnabled then
        ...
        Property.Item.Description.PerformLayout = function()
            ...
            Property.Item.Description:SetText( Arivia.CustomEntityDescriptions[Property.Value.entity] and Arivia.CustomEntityDescriptions[Property.Value.entity] .. "\n\n\n" or text )
        end
    ...
    Property.Item.ButtonAction.PerformLayout = function()
        if AriviaAllowGunPurchase(Property.Value) then
            Property.Item.ButtonAction.Text = string.upper( Arivia.Language.MakePurchase .. ": " .. GAMEMODE.Config.currency .. ( Property.Value.pricesep or Property.Value.price ) )
