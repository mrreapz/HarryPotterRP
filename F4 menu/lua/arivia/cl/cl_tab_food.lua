...
function PANEL:Init()
    ...
    for i = 0, 0 do
        for k, v in ipairs(FoodItems) do
            ...
            if !AriviaAllowFoodPurchase(v) then continue end
            ...
            function ListItem:Paint( w, h )
                ...
                local color = Arivia.TabFood.buttonColor
                local txtColor = textNormal
                if ListItem:IsHovered() or ListItem:IsDown() then 
                    color = Arivia.TabFood.buttonHoverColor
                    txtColor = textHover
                end
                ...
                if Arivia.TabFood.xpsystemEnabled and LevelSystemConfiguration then
                ...
                if LevelSystemPrestigeConfiguration and Arivia.TabFood.prestigeEnabled then
                ...
                if (Arivia.TabFood.xpsystemEnabled and ( LevelSystemConfiguration ) ) or (Arivia.TabFood.prestigeEnabled and ( LevelSystemPrestigeConfiguration or v.prestige ) )  then
                    originNameX = 11
                end
                ...
                draw.DrawText( objectName, "AriviaFontObjectListName", 65, originNameX, Arivia.TabFood.nameColor or Color( 255, 240, 244 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )

                if Arivia.TabFood.xpsystemEnabled then
                    if LevelSystemConfiguration then
                        ...
                        if Arivia.TabFood.prestigeEnabled and LevelSystemPrestigeConfiguration then
                            draw.DrawText( prestigeText, "AriviaFontObjectLevel", 140, 35, Color( 255, 240, 244, 100 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
                        end
                    end
                elseif Arivia.TabFood.prestigeEnabled then
    ...
    if Arivia.CustomEntityDescriptionsEnabled then
        ...
        Property.Item.Description.PerformLayout = function()
            ...
            Property.Item.Description:SetText( Arivia.CustomEntityDescriptions[foodEntity] and Arivia.CustomEntityDescriptions[foodEntity] .. "\n\nEnergy: " .. foodEnergy .. "\n\n\n" or text )
    ...
    Property.Item.ButtonAction.PerformLayout = function()
        if AriviaAllowFoodPurchase(Property.Value) then
...
vgui.Register("AriviaTabFood", PANEL, "DPanel")